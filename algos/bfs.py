def bfs(neighbors_func, start, startdata=None, parents=None, yield_edges=False):
    """ Performs a BFS traversal of a graph from a given node.
    We expect the graph to contain the following:

    neighbors_func: (node) => [child nodes]     - Returns the neighbors of a given node.

    Some details are:
    1. Does a direct BFS.  
    2. Yields both nodes and edges as traversed and adds "path" to a parents list
    ...
    while queue:
        level += 1
        newqueue = []
        for currNode, currData in queue:
            if yield_edges:
                result = yield "node", (currNode, level, currData)
            else:
                result = yield (currNode, level, currData)
            if result is False:
                # Then ignore the children for this node!
                continue

            for childNode, childData in neighbors_func(currNode, currData, parents):
                # Process the edge between node and neighbour
                if yield_edges:
                    result = yield "edge", (currNode, childNode, childData)
                    if result is False:
                        return
                ...
                if state.get(childNode, UNDISCOVERED) == UNDISCOVERED:
                    ...
                    newqueue.append((childNode, childData))
